...
function openMultiOrderModal() {
  ...
  if (window.innerWidth <= 1026) {
    // 1. Убираем все старые спейсеры и грабберы
    multiModal.querySelectorAll('.top-spacer, .ios-grabber-bottom').forEach(el => el.remove());

    // 2. Минимальный верхний отступ — текст "Ваши заказы" теперь почти у самого верха
    const topSpacer = document.createElement('div');
    topSpacer.className = 'top-spacer';
    topSpacer.style.cssText = `
      height: 36px;                    /* было 56px → теперь текст сильно выше */
      padding-top: env(safe-area-inset-top);
      flex-shrink: 0;
      pointer-events: none;
    `;
    modalInner.insertBefore(topSpacer, modalInner.firstChild);

    // 3. Ещё чуть поднимаем сам заголовок (опционально, но красиво)
    const titleWrapper = modalInner.querySelector('div[style*="padding:0 2rem"]') || 
                         modalInner.children[1]; // обычно второй ребёнок после спейсера
    if (titleWrapper) {
      titleWrapper.style.paddingTop = '4px';
    }

    // 4. Компактная кнопка "Закрыть" + нижний граббер
    const closeBlock = modalInner.querySelector('div[style*="padding:1rem 2rem 2rem"]') ||
                       modalInner.querySelector('div[style*="padding-bottom"]');
    if (closeBlock) {
      closeBlock.style.padding = '10px 20px 8px 20px';
      const btn = closeBlock.querySelector('button');
      if (btn) {
    ...
    // 5. Нижний граббер — аккуратный и только один
    const grabber = document.createElement('div');
    grabber.className = 'ios-grabber-bottom';
    grabber.style.cssText = `
      ...
      flex-shrink: 0;
    `;
    modalInner.appendChild(grabber);

    // 6. Максимум места под список заказов
    const list = document.getElementById('ordersVerticalList');
    if (list) {
      list.style.flex = '1';
      list.style.overflowY = 'auto';
      list.style.padding = '0 18px';
      list.style.margin = '0 -18px';
      list.style.scrollbarWidth = 'none';
      list.style.webkitOverflowScrolling = 'touch';
    }

    // 7. Финальные отступы
    modalInner.style.paddingBottom = 'max(14px, env(safe-area-inset-bottom))';
    modalInner.style.display = 'flex';
    modalInner.style.flexDirection = 'column';

    // Анимация появления
    requestAnimationFrame(() => multiModal.classList.add('show'));
  }

  // Рендерим заказы сразу
